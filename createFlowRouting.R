...
# bounded by GIS mask
	...
	for(ii in patchNeighbourRC_LEN){
		...
		allNeighbourInfo = simplify2array(tapply(neighbourLength, INDEX=neighbourOrder, function(jj){
			...
			return <- c(
				patchID = neighbor_patch_info['patchID'], #patchID, zone, hill [1,2,3]
                zoneID = neighbor_patch_info['zoneID'],
                hillID = neighbor_patch_info['hillID'],
				## ... distance
				dist = sqrt((neighbor_patch_info['xx']-current_patch_info['xx'])^2 +
					(neighbor_patch_info['yy']-current_patch_info['yy'])^2), # distance [4]
				## ... rise
				rise = idiffDEM, #rise (local prefer) [5] # zero correct
