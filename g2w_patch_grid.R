...
#----------------------------------------------------------------------------------------------
	param = read.csv("~/Dropbox/Myself/passing/genericVeg.csv",skip=4,header=T,stringsAsFactors=F)
...
#----------------------------------------------------------------------------------------------
	outWorldFile = 'world_testing.csv'
	#latitude = 39.363142 
...
#----------------------------------------------------------------------------------------------
	...
	isohyetMAP = 'isohyet' ##<<--------- [can it be 0 or negative?]
	imperviousMAP = 'impervious'
	laiMAP = 'lai'
	VegCoverMAP = ''
	VegGapMAP = ''
	...
	patchIMP = tapply(dem[mask],INDEX=patch[mask], FUN=mean)
...
#----------------------------------------------------------------------------------------------		
	outWorldFile = '~/Dropbox/Myself/passing/BES_SLB/world_testing.csv'
	title = c(defaultWorldName, defaultBasinName, defaultHillName, defaultZoneName, defaultPatchName, defaultStratumName)
	write( title, outWorldFile, ncolumns=length(title), append=F, sep=',')
		
		
	WorldBasinColumn = rep(1, numpatch) %o% c(1,1, mean(rast$x[mask]), mean(rast$y[mask]), mean(rast$dem1[mask]), 1, latitude, 0)
	hillColumn = cbind(
