...
#----------------------------------------------------------------------------------------------
	...
	rast3 = readRAST(c('crossline','str62ha2'))
		crossSite = rast3@data[[1]][mask] ##<<----
		strchannel = rast3@data[[2]][mask]
	...
	##-----<< order >>------# 
	len = sum(mask)
	RHESSysOrder = order(hill,zone,patch) 
	tapplyPatchOrder = rep(NA,sum(mask)); tapplyPatchOrder[RHESSysOrder] = match(patch[RHESSysOrder], unique(patch[RHESSysOrder])) ## why not using this for patch tapply? then we don't need to resort.
	#tapplyZoneOrder = rep(NA,sum(mask)); tapplyZoneOrder[RHESSysOrder] = match(zone[RHESSysOrder], unique(zone[RHESSysOrder]))
	#tapplyHillOrder = rep(NA,sum(mask)); tapplyHillOrder[RHESSysOrder] = match(hill[RHESSysOrder], unique(hill[RHESSysOrder]))
	...
	patchVegnum = tapply(1:sum(mask),INDEX=tapplyPatchOrder, FUN=function(x){
			
			strFrac = sum(!is.na(strchannel[x]))/length(x)
			subGridAssignment[1,] = c(mean(forestFrac[x])-strFrac, mean(lawnFrac[x])-strFrac, mean(impervious[x]) )
			subGridAssignment[1,subGridAssignment[1,]<0] = 0
			...
			patchOUT_singal = mean(crossSite[x],na.rm=T);
			#land = landuseClass[which.max(subGridAssignment[1,])]
			land = ifelse(subGridAssignment[1,1]==0 & subGridAssignment[1,2]==0,3,ifelse(subGridAssignment[1,2]==0,2,1))
			land = land + ifelse(is.na(patchOUT_singal), 0,600) ## use crossSite class to select patch output
...
#----------------------------------------------------------------------------------------------		
	...
	##--------------------- compare to the previous worldfile
	# world1 = read.csv('worldfiles/worldfile.csv') # 102331 rows
	# world2 = read.csv('worldfiles/worldfile2.csv') # 102305 rows
	
	# world1_pcount = table(world1[,'patchID']) #length = 91649
	# world2_pcount = table(world2[,'patchID']) #length = 91649
	
	# sum(names(world1_pcount) == names(world2_pcount))
	# write.csv( cbind(names(world1_pcount), names(world2_pcount)), 'check.csv', row.names=F)
	
	# cond = world1_pcount != world2_pcount
	# world1_pcount[cond]
# # 1708866 1708867 1708868 1708869 1708870 1711044 1711045 1713224 1728482 1730661 
      # # 2       2       2       2       2       2       2       2       2       2 
# # 1730662 1732754 1732776 1732777 1732778 1732779 1732780 1732783 1732812 1732842 
      # # 2       2       2       2       2       2       2       2       2       2 
# # 1734956 1734996 1735022 1737202 1739365 1739366 
      # # 2       2       2       2       2       2 
...
# # 1734956 1734996 1735022 1737202 1739365 1739366 
	...
	# world1[world1[,'patchID']==1708866, ] # 2, 3
	# world2[world2[,'patchID']==1708866, ] # 4
